...
func (in *CcInstallConfig) DeepCopyInto(out *CcInstallConfig) {
	*out = *in
	...
	if in.PreinstallCmd != nil {
		in, out := &in.PreinstallCmd, &out.PreinstallCmd
		*out = make([]string, len(*in))
		copy(*out, *in)
	}
	if in.PreInstallEnvironmentVariables != nil {
		in, out := &in.PreInstallEnvironmentVariables, &out.PreInstallEnvironmentVariables
		*out = make([]corev1.EnvVar, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
	}
	if in.PreInstallVolumeMounts != nil {
		in, out := &in.PreInstallVolumeMounts, &out.PreInstallVolumeMounts
		*out = make([]corev1.VolumeMount, len(*in))
		for i := range *in {
			(*in)[i].DeepCopyInto(&(*out)[i])
		}
